				...
				$scope.GenerarTicketTurno = function(BtnSeleccionado){
					...
					$http.post("index.php?c=ponche&a=GenerarTurno", {Action: "GenerarTurno",BotonID: $scope.BotonID,  SucursalID: $scope.sucursalID}).then(function (response) {
						...
						if(response.data[0]){
							...
							$scope.MaquetaPrint = "<html><body  style='font-family:Calibri;'><center><img src='uploads/"+$scope.empresaLogo+"'  height='70'  width='150'><br><b> "+$scope.empresa+"</b><b style='font-family:Calibri;font-size:0.8em;'><br>"+$scope.sucursal+"-"+$scope.departamento+"</b><br> ____________________<br>TURNO</div><br><div style='font-family:Calibri;font-size:2em;'>"+$scope.ValorTurnoBoton+"-"+$scope.NuevoTurno+"</div><div style='font-family:Calibri;'> ____________________</div><p>"+moment().format('DD/MM/YYYY, h:mm:ss A');+"</p></center></body></html>";
